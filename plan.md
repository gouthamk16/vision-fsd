...
- [x] **Object velocity (predicted boxes)** (Priority 3): `PredictionVelocityTracker` does greedy nearest-by-class association in the global frame over CenterPoint boxes (no `instance_token`), then the same finite-difference velocity + constant-velocity prediction. Wired into `BevWorldModel.pred_objects`; `world_bev --predictions` draws orange pred velocity arrows alongside the cyan GT ones. Validated on scene 0 vs the GT oracle: 59/70 moving objects matched, velocity-vector error mean 0.27 / median 0.25 m/s, speed error mean 0.18 m/s. Optional Kalman smoother deferred (errors are already low).
...
Object detection is done (camera frustum fusion). The dynamic branch now has velocity + short-horizon prediction from both the GT oracle and tracked CenterPoint detections (validated to ~0.25 m/s of the oracle). The static side (occupancy, height, soon lanes) is in good shape; remaining big items are lane topology and wiring predicted objects into the planner's collision check.
