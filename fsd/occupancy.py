...
def save_occupancy_sequence(
    dataroot: str | Path | None = None,
    scene_index: int = 0,
    scene_name: str | None = None,
    start_sample_index: int = 0,
    max_frames: int | None = 40,
    output_path: str | Path = "outputs/nuscenes_occupancy_scene0_40f.mp4",
    fps: float = 2.0,
    resolution: float = 0.25,
    scale: int = 2,
) -> Path:
    loader = NuScenesSceneLoader(dataroot=dataroot)
    mapper = TemporalOccupancyMapper(resolution=resolution)
    ...
    try:
        for frame, lidar in loader.iter_scene_frames(
            ...
            prob = mapper.step(lidar)
            image = render_occupancy_bev(frame, prob, resolution=resolution, scale=scale)
            rendered += 1
            if writer is None:
    ...
    if rendered == 0:
        raise RuntimeError("No occupancy frames were rendered")
    return output


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render temporal log-odds occupancy BEV from nuScenes.")
    ...
    parser.add_argument("--resolution", type=float, default=0.25, help="Occupancy grid meters per cell.")
    parser.add_argument("--scale", type=int, default=2, help="Nearest-neighbor scale for saved output.")
    parser.add_argument("--output", default="outputs/nuscenes_occupancy_scene0_40f.mp4", help="Output video path.")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    output = save_occupancy_sequence(
